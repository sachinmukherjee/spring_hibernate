	...
	<div class="row">
	 <div class="col-md-12">
		<form action="${pageContext.request.contextPath}/owner_company/submit/" method="POST" modelAttribute="ownerCompany" class="form-control">
		    ...
		    <form:hidden path="ownerCompany.id" />
			</br></br>
	  	...
	  	<table class="table table-responsive">
	  	  <thead>
	  		...
	  		<c:forEach items="${ownerCompany.ownerCompanyOffices}" varStatus="row">
	  		   		<tr class="toClone">
		  		 		<td>
		  		 			<form:input path="ownerCompany.ownerCompanyOffices[${row.index}].name"/>
		  		 			<form:hidden path="ownerCompany.ownerCompanyOffices[${row.index}].id" cssClass="office"/>
		  		 			<form:hidden path="ownerCompany.ownerCompanyOffices[${row.index}].ownerCompany.id" cssClass="company"/>
		  		 		</td>
...
var counter = $(".toClone").length;
$(document).on("click",".addMore", function(){
	    var clonedRow = $(".toClone:first").clone();
	    $(clonedRow).find(".office").remove();
